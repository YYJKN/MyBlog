        ...
        <div>
          <div class="item" v-for="item in lists" :key="item.id">
            ...
            <div class="right">
              ...
              <div class="dibu">
                <div class="icon">
                  <span><i class="el-icon-date">{{item.article.createdAt.split('T')[0]}}</i></span>
...
export default {
  ...
  data() {
    return {
      lists: [],
      isActive: 0,
      arr:[['时间','id'],['热度','views'],['评论','comments'],['收藏','stars']],
      isLoading: true,
      total: 100,
      size: 5,
      page: 1,
  ...
  methods: {
    ...
    async getPages(category = 'id',index=0) {
      let sort = {type:category}
      this.isActive=index
      const res = await this.$article.post("/getPages", {
        type: this.type,
        size: this.size,
        page: this.page,
        sort
