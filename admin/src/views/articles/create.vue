  ...
  export default {
    ...
    methods: {
        ...
        this.$refs.ruleForm.validate(async (valid) => {
          if (valid) {
            if (this.$route.params.id) {
              const res = await this.$article.put(
              ...
              const res = await this.$article.post("/create", this.form);
              if (res.status == 200) {
      ...
      async queryArticle(id) {
        const res = await this.$article.get("/edit/" + id);
    ...
    created() {
      if (this.$route.params.id) this.queryArticle(this.$route.params.id);
    },
  };
</script>
